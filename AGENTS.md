...
1. `npm run generate-proto`: Generates Go and TypeScript files from `.proto` definitions. This includes:
   - Go: `protoc --go_out=./generated/go --go_opt=paths=source_relative ...` (or `python -m grpc_tools.protoc ...` as per current `package.json`)
...
This will run `ruff` and `mypy` for any remaining Python utility scripts (e.g., within `ai_sads_poc/`), and `prettier` for formatting various file types. MyPy checks related to generated Python protobufs are no longer applicable as these are not generated by the main build.
