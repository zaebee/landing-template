...
function parseSadsAttributeString(
  ...
  switch (sadsPropertyKey.toLowerCase()) {
    ...
    case "margin":
    case "gap":
      const spacingTokenKey =
        `SPACING_TOKEN_${rawValue.toUpperCase()}` as keyof typeof SadsSpacingToken;
    ...
    case "bordercolor":
      const colorTokenKey =
        `COLOR_TOKEN_${rawValue.toUpperCase()}` as keyof typeof SadsColorToken;
      ...
      break;
    case "fontweight":
      const fontWeightTokenKey =
        `FONT_WEIGHT_TOKEN_${rawValue.toUpperCase()}` as keyof typeof SadsFontWeightToken;
      if (SadsFontWeightToken[fontWeightTokenKey] !== undefined) {
        attributeValue.fontWeightToken =
          SadsFontWeightToken[fontWeightTokenKey];
      } else {
        attributeValue.customValue = rawValue;
      }
      break;
    case "borderradius":
      const radiusTokenKey =
        `BORDER_RADIUS_TOKEN_${rawValue.toUpperCase()}` as keyof typeof SadsBorderRadiusToken;
      if (SadsBorderRadiusToken[radiusTokenKey] !== undefined) {
        attributeValue.borderRadiusToken =
          SadsBorderRadiusToken[radiusTokenKey];
      } else {
        attributeValue.customValue = rawValue;
      }
      break;
...
function extractBaseStylesFromDataset(dataset: DOMStringMap): SadsStylingSet {
  ...
  for (const key in dataset) {
    if (
      ...
      if (rawValue !== undefined) {
        const parsedValue = parseSadsAttributeString(
          normalizedSadsKey,
          rawValue
        );
        if (parsedValue) {
          if (!stylingSet.attributes) {
            // Should be initialized by type if it's a map
...
interface RawResponsiveRuleInput {
  // Type for the input JSON structure from data-sads-responsive-rules
  breakpoint: string;
  ...
  try {
    ...
    rawParsedRules.forEach((rawRule) => {
      ...
      for (const sadsKey in rawRule.styles) {
        ...
        if (parsedValue) {
          if (!stylingSetForRule.attributes) {
    ...
    console.error(
      `SADS TS: Error parsing responsive rules: "${rulesString}"`,
      e
    );
  }
  return result;
...
## 5. Backend API for AI-Assisted SADS Generation

As part of exploring advanced SADS capabilities, a backend API endpoint (`/api/generate-sads-from-nl`) has been implemented in Go (`main.go` using `api_handlers.go`). This endpoint serves the `public/nl-sads-test.html` page by providing a way to convert natural language style prompts into SADS attributes using LLMs (OpenAI or Mistral).

- **Purpose:** Allows interactive generation of SADS attributes from natural language.
- **Workflow:**
  1.  Client (`nl-sads-test.html`) sends HTML snippet, style prompt, theme context (JSON string), provider, and model to the Go backend.
  2.  Go backend constructs a detailed prompt and calls the selected LLM API.
  3.  LLM returns a string of `data-sads-*` attributes.
  4.  Go backend parses this string into a structured map (conceptually matching `SadsStylingSet`) and returns both the raw string and the parsed map to the client.
- **Relevance to Proto Integration:** The "parsed map" returned by the Go backend is designed to align with the `SadsStylingSet` and `SadsAttributeValue` proto definitions. This demonstrates a practical application where AI-generated styles are immediately structured in a way that's compatible with the SADS proto schema, facilitating potential future consumption by a SADS engine that natively understands these proto structures.
- **Current Status:** The Go backend API is implemented. The next step for this feature would be to implement the JavaScript logic in `public/nl-sads-test.html` to call this API and utilize its response.
