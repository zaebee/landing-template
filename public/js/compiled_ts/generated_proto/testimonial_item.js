...
class TestimonialItem$Type extends MessageType {
        ...
        while (reader.pos < end) {
            ...
            switch (fieldNo) {
                ...
                case /* website_content.v1.I18nString author */ 2:
                    message.author = I18nString.internalBinaryRead(reader, reader.uint32(), options, message.author);
                    break;
                case /* website_content.v1.Image author_image */ 3:
                    message.authorImage = Image.internalBinaryRead(reader, reader.uint32(), options, message.authorImage);
                    break;
                default:
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        if (message.author)
            I18nString.internalBinaryWrite(message.author, writer.tag(2, WireType.LengthDelimited).fork(), options).join();
        /* website_content.v1.Image author_image = 3; */
        if (message.authorImage)
            Image.internalBinaryWrite(message.authorImage, writer.tag(3, WireType.LengthDelimited).fork(), options).join();
