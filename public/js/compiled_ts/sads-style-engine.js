...
class SADSEngine {
    constructor(customThemeConfig = {}, externalDefaultTheme = null) {
        ...
        if (!baseTheme || Object.keys(baseTheme).length === 0) {
            ...
            baseTheme = {
                ...
                fontStyle: {},
                borderStyle: {},
            }; // Cast needed due to partial empty structure
        }
    ...
    applyStylesTo(rootElement) {
        ...
        elementsToStyle.forEach((el) => {
            ...
            const responsiveStyles = this._parseResponsiveRules(attributes.sadsResponsiveRules, // sadsResponsiveRules might be undefined
            targetSelector);
            for (const [bpQuery, cssRules] of Object.entries(responsiveStyles)) {
                if (cssRules)
                    this._addCssRule(targetSelector, cssRules, bpQuery);
            }
        });
    }
    _addCssRule(selector, rules, mediaQuery = null) {
        if (!rules.trim())
            return; // Do not add empty rules
    ...
    _mapSemanticValueToActual(cssProperty, semanticValue) {
        if (semanticValue === null || semanticValue === undefined)
            return null;
        if (cssProperty === null)
            return null; // Cannot map value if CSS property is unknown or irrelevant
