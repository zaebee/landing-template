document.addEventListener("DOMContentLoaded", () => {
  ...
  const componentRenderTarget = document.getElementById(
  ...
  if (
    ...
    typeof sadsManager === "undefined" ||
    typeof SADS_DEFAULT_THEME === "undefined"
  ) {
    console.error(
      "SADS Engine, Manager, or Default Theme not loaded. Previewer may not function correctly."
    );
    componentRenderTarget.innerHTML =
  ...
  fetch("/api/sads/components")
    ...
    .then((components) => {
      componentSelector.innerHTML =
        '<option value="">-- Select a Component --</option>'; // Clear loading message
      components.forEach((componentName) => {
        const option = document.createElement("option");
        option.value = componentName;
        option.textContent =
          componentName.charAt(0).toUpperCase() + componentName.slice(1); // Capitalize
  ...
  componentSelector.addEventListener("change", (event) => {
    ...
    fetch(`/api/sads/component/${componentName}`)
      ...
      .then((htmlContent) => {
        ...
        if (
          sadsManager &&
          typeof sadsManager.reapplySadsStyles === "function"
        ) {
          sadsManager
            .reapplySadsStyles()
            .then(() => {
  ...
  componentRenderTarget.addEventListener("click", (event) => {
    ...
    ); // Try to get a relevant SADS element
