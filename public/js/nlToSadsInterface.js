class NLToSadsInterface {
    ...
    applyAttributesToTarget(attributes) {
        ...
        console.log("NLToSadsInterface: Attempting to call sadsEngine.applyStylesTo. sadsEngine instance:", this.sadsEngine); // New Log
        if (this.sadsEngine &&
            typeof this.sadsEngine.applyStylesTo === "function") {
            this.sadsEngine.applyStylesTo(this.targetElement);
        }
        else {
            console.error("NLToSadsInterface: sadsEngine is not available or applyStylesTo is not a function!", this.sadsEngine);
        }
    }
}
