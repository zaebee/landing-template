(async () => {
  try {
    ...
    const response = await fetch(
      `public/generated_configs/config_${lang}.json`
    );
    if (!response.ok) {
      throw new Error(
        `Failed to fetch config_${lang}.json: ${response.statusText}`
      );
    }
    ...
    if (
      config &&
      config.block_data_loaders &&
      config.block_data_loaders["components/contact-form/contact-form.html"]
    ) {
      const contactFormLoaderConfig =
        config.block_data_loaders["components/contact-form/contact-form.html"];
      if (contactFormLoaderConfig.data_file) {
        const contactDataResponse = await fetch(
          contactFormLoaderConfig.data_file
        );
        if (!contactDataResponse.ok) {
          throw new Error(
            `Failed to fetch ${contactFormLoaderConfig.data_file}: ${contactDataResponse.statusText}`
          );
        }
      ...
      console.warn(
        "Contact form configuration not found in site config or structure is unexpected."
      );
      window.contactFormConfig = {}; // Provide a default empty object
    }
    ...
    if (config && config.base_url) {
      ...
      if (!window.siteConfig.base_url) {
        ...
        console.log(
          "siteConfig.base_url was not found in config_*.json, defaulted to '/'"
        );
      }
    }
  } catch (error) {
