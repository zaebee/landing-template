...
class ImageSlider {
  ...
  constructor(sliderElement: HTMLElement) {
    ...
    this.slidesContainer = sliderElement.querySelector<HTMLElement>(SLIDES_CONTAINER_SELECTOR) as HTMLElement;
    this.slides = Array.from(sliderElement.querySelectorAll<HTMLElement>(SLIDE_SELECTOR));
    this.prevButton = sliderElement.querySelector<HTMLButtonElement>(PREV_BUTTON_SELECTOR);
    this.nextButton = sliderElement.querySelector<HTMLButtonElement>(NEXT_BUTTON_SELECTOR);
    this.dotsContainer = sliderElement.querySelector<HTMLElement>(DOTS_CONTAINER_SELECTOR);
    this.dots = this.dotsContainer ? Array.from(this.dotsContainer.querySelectorAll<HTMLButtonElement>(DOT_SELECTOR)) : [];

    this.totalSlides = this.slides.length;
    this.autoplayEnabled = sliderElement.dataset.sadsAutoplay === "true";
    this.autoplayInterval = parseInt(sliderElement.dataset.sadsAutoplayInterval || "5000", 10);
  ...
  private init(): void {
    ...
    if (this.totalSlides <= 1) {
        ...
        if (this.autoplayEnabled) {
            ...
            this.sliderElement.addEventListener("focusin", () => this.pauseAutoplay()); // Pause if user tabs into slider elements
            this.sliderElement.addEventListener("focusout", () => this.startAutoplay()); // Resume if focus leaves slider
        }
    }
  ...
  private showSlide(index: number): void {
    ...
    this.slides.forEach((slide, i) => {
      slide.setAttribute("data-sads-display", i === this.currentIndex ? "flex" : "none");
      // Update ARIA attributes for accessibility
      slide.setAttribute("aria-hidden", i === this.currentIndex ? "false" : "true");
      const img = slide.querySelector<HTMLImageElement>('[data-sads-element="slide-image"]');
      if(img) {
    ...
    if(this.prevButton) {
        ...
        this.prevButton.setAttribute("data-sads-opacity", this.currentIndex === 0 ? "custom:0.5" : "custom:1");
    }
    if(this.nextButton) {
        this.nextButton.disabled = this.currentIndex === this.totalSlides - 1;
        this.nextButton.setAttribute("data-sads-opacity", this.currentIndex === this.totalSlides - 1 ? "custom:0.5" : "custom:1");
    }
...
export function initSliderComponent(): void {
  const sliderElements = document.querySelectorAll<HTMLElement>(SLIDER_COMPONENT_SELECTOR);
  if (sliderElements.length > 0) {
    sliderElements.forEach(sliderEl => new ImageSlider(sliderEl));
    console.log(`Image Slider Component(s) Initialized: ${sliderElements.length} found.`);
  }
}
