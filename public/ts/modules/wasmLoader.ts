...
(function () {
  window.sadsPocWasmReadyPromise = new Promise<SadsPocWasm>(
    async (resolve, reject) => {
      ...
      try {
        ...
        if (typeof window.Go === "undefined") {
          console.error(
            "wasmLoader.ts: Go global object not defined after loading wasm_exec.js."
          );
        ...
        console.log(
          `wasmLoader.ts: Attempting to instantiate WASM module from: ${sadsPocPath}`
        );
        ...
        setTimeout(() => {
          if (
            ...
            (typeof window.sadsPocWasm.resolveColor === "function" ||
              typeof window.sadsPocWasm.resolveSadsValue === "function")
          ) {
            console.log(
              "wasmLoader.ts: WASM module loaded and sadsPocWasm functions are available."
            );
            resolve(window.sadsPocWasm);
          } else {
            console.error(
              "wasmLoader.ts: sadsPocWasm functions not found after WASM instantiation."
            );
            reject(
              new Error(
                "sadsPocWasm functions not found after WASM instantiation."
              )
            );
          }
        }, 100);
        ...
        console.error(
          "wasmLoader.ts: Error during WASM loading or instantiation:",
          error
        );
        reject(error);
      }
  ...
  window.sadsPocWasmReadyPromise.catch((error) => {
    console.warn(
      "wasmLoader.ts: sadsPocWasmReadyPromise was rejected.",
      error.message
    );
  });
})();
