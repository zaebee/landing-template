...
class SADSEngine {
  ...
  private _initializeTheme(
    ...
    const clonedDefaultConfig: SadsTheme = JSON.parse(JSON.stringify(defaultConfig));
    ...
    return this._deepMergeThemes(clonedDefaultConfig, customThemeConfig) as SadsTheme;
  }
  ...
  private _createDynamicStyleSheet(): CSSStyleSheet | null {
    ...
    let styleEl = document.getElementById("sads-dynamic-styles") as HTMLStyleElement | null;
    if (!styleEl) {
  ...
  private _parseSadsAttributeString(sadsPropertyKey: string, rawValue: string): SadsAttributeValue | null {
    const attributeValue: PartialMessage<SadsAttributeValue> = { valueType: { oneofKind: undefined } };
    if (rawValue.startsWith("custom:")) {
      attributeValue.valueType = { oneofKind: "customValue", customValue: rawValue.substring("custom:".length) };
      return SadsAttributeValue.create(attributeValue);
    }
    const normalizedKey = sadsPropertyKey.toLowerCase();
    switch (normalizedKey) {
      case "padding": case "paddingtop": case "paddingbottom": case "paddingleft": case "paddingright":
      case "margin": case "margintop": case "marginbottom": case "marginleft": case "marginright":
      case "gap": case "borderwidth":
        const spacingTokenKey = `SPACING_TOKEN_${rawValue.toUpperCase()}`;
        if (spacingTokenKey in SadsSpacingToken) {
          attributeValue.valueType = { oneofKind: "spacingToken", spacingToken: SadsSpacingToken[spacingTokenKey as keyof typeof SadsSpacingToken] };
        } else {
          attributeValue.valueType = { oneofKind: "customValue", customValue: rawValue };
        }
        break;
      case "bgcolor": case "textcolor": case "bordercolor":
        const colorTokenKey = `COLOR_TOKEN_${rawValue.toUpperCase()}`;
        if (colorTokenKey in SadsColorToken) {
          attributeValue.valueType = { oneofKind: "colorToken", colorToken: SadsColorToken[colorTokenKey as keyof typeof SadsColorToken] };
        } else {
          if (rawValue.toLowerCase() === 'transparent') {
             attributeValue.valueType = { oneofKind: "colorToken", colorToken: SadsColorToken.COLOR_TOKEN_TRANSPARENT };
          } else {
            attributeValue.valueType = { oneofKind: "customValue", customValue: rawValue };
          }
        }
      ...
      case "fontweight":
        ...
        if (fontWeightTokenKey in SadsFontWeightToken) {
          attributeValue.valueType = { oneofKind: "fontWeightToken", fontWeightToken: SadsFontWeightToken[fontWeightTokenKey as keyof typeof SadsFontWeightToken] };
        } else {
          attributeValue.valueType = { oneofKind: "customValue", customValue: rawValue };
        }
        break;
      case "borderradius":
        ...
        if (radiusTokenKey in SadsBorderRadiusToken) {
          attributeValue.valueType = { oneofKind: "borderRadiusToken", borderRadiusToken: SadsBorderRadiusToken[radiusTokenKey as keyof typeof SadsBorderRadiusToken] };
        } else {
          attributeValue.valueType = { oneofKind: "customValue", customValue: rawValue };
        }
        break;
      case "fontsize":
        attributeValue.valueType = { oneofKind: "fontSizeValue", fontSizeValue: rawValue };
        break;
      default:
        attributeValue.valueType = { oneofKind: "customValue", customValue: rawValue };
        break;
  ...
  private _extractBaseStylesFromDataset(dataset: DOMStringMap): SadsStylingSet {
    ...
    for (const key in dataset) {
      if ( Object.prototype.hasOwnProperty.call(dataset, key) && key.startsWith("sads") &&
           key.toLowerCase() !== "sadsresponsiverules" && key.toLowerCase() !== "sadscomponent" && key.toLowerCase() !== "sadselement") {
  ...
  private _parseResponsiveRulesTS(rulesString: string | undefined): ProtoSadsResponsiveStyle[] {
  ...
  private _parseElementDatasetToSadsElementStyles(dataset: DOMStringMap): SadsElementStyles | null {
    const baseStylesSet: SadsStylingSet = this._extractBaseStylesFromDataset(dataset);
    const responsiveRulesString = dataset.sadsResponsiveRules;
    const responsiveStylesArray: ProtoSadsResponsiveStyle[] = this._parseResponsiveRulesTS(responsiveRulesString);
    const hasBaseStyles = baseStylesSet.attributes && Object.keys(baseStylesSet.attributes).length > 0;
  ...
  private _resolveSadsAttributeValueToCss(cssProperty: string | null, sadsValue: SadsAttributeValue): string | null {
    ...
    switch (sadsValue.valueType.oneofKind) {
      case "customValue":
        ...
        if (semanticKey.startsWith("custom:")) return semanticKey.substring("custom:".length);
        break;
      case "spacingToken":
        const spacingTokenStr = SadsSpacingToken[sadsValue.valueType.spacingToken]?.replace('SPACING_TOKEN_', '').toLowerCase();
        semanticKey = spacingTokenStr === "unspecified" ? undefined : spacingTokenStr;
        valueFromToken = true;
        break;
      case "colorToken":
        const colorTokenStr = SadsColorToken[sadsValue.valueType.colorToken]?.replace('COLOR_TOKEN_', '').toLowerCase();
        semanticKey = colorTokenStr === "unspecified" ? undefined : colorTokenStr;
        if (semanticKey === "transparent") return "transparent";
        valueFromToken = true;
        break;
      case "fontWeightToken":
        const fontWeightStr = SadsFontWeightToken[sadsValue.valueType.fontWeightToken]?.replace('FONT_WEIGHT_TOKEN_', '').toLowerCase();
        semanticKey = fontWeightStr === "unspecified" ? undefined : fontWeightStr;
        valueFromToken = true;
        break;
      case "borderRadiusToken":
        const radiusTokenStr = SadsBorderRadiusToken[sadsValue.valueType.borderRadiusToken]?.replace('BORDER_RADIUS_TOKEN_', '').toLowerCase();
        semanticKey = radiusTokenStr === "unspecified" ? undefined : radiusTokenStr;
        valueFromToken = true;
        break;
      case "fontSizeValue":
    ...
    if (semanticKey === undefined || semanticKey === null || semanticKey === "unspecified") return null;
    ...
    const themeCategoryMap: { [cssProp: string]: keyof SadsTheme | null } = {
      ...
      gap: "spacing", "border-width": "spacing", width: "spacing", height: "spacing",
    ...
    if (categoryKey) {
      ...
      if (themeCategory) {
        if (categoryKey === "colors") {
          ...
          return themeCategory[colorThemeKey] || themeCategory[semanticKey] || (valueFromToken ? null : semanticKey);
        }
  ...
  private _generateCssFromSadsStylingSet(stylingSet: SadsStylingSet): string {
    ...
    for (const sadsKey in stylingSet.attributes) {
      if (Object.prototype.hasOwnProperty.call(stylingSet.attributes, sadsKey)) {
        ...
        if (actualValue !== null) cssText += `${cssProperty}: ${actualValue};\n`;
      }
    }
    return cssText;
  }

  private _applyProcessedStylesToElement(el: HTMLElement, elementSadsConfig: SadsElementStyles): void {
    const targetSelector = this._getTargetSelector(el);
    if (elementSadsConfig.baseStyles && elementSadsConfig.baseStyles.attributes) {
    ...
    if (elementSadsConfig.responsiveStyles && elementSadsConfig.responsiveStyles.length > 0) {
      elementSadsConfig.responsiveStyles.forEach((responsiveStyle: ProtoSadsResponsiveStyle) => {
        if (responsiveStyle.breakpointKey && responsiveStyle.styles && responsiveStyle.styles.attributes) {
          const mediaQuery = this.theme.breakpoints[responsiveStyle.breakpointKey] || responsiveStyle.breakpointKey;
  ...
  public applyStyles(): void {
    ...
    document.querySelectorAll<HTMLElement>("[data-sads-component]").forEach((rootEl) => {
      const elementsToStyle: HTMLElement[] = [ rootEl, ...Array.from(rootEl.querySelectorAll<HTMLElement>("[data-sads-element]"))];
  ...
  private _parseResponsiveRules(rulesString: string | undefined, targetSelector: string): { [mediaQuery: string]: string } {
    const responsiveStyles: { [mediaQuery: string]: string } = {};
    if (!rulesString) return responsiveStyles;
    try {
      const parsedRulesOld: { breakpoint: string, styles: { [key:string]: string} }[] = JSON.parse(rulesString);
      parsedRulesOld.forEach((rule) => {
        ...
        for (const [respSadsPropKey, respSemanticVal] of Object.entries(rule.styles)) {
          ...
          if (cssProp && actualVal !== null) responsiveStyles[targetQuery] += `${cssProp}: ${actualVal} !important;\n`;
          else if (!cssProp) console.warn(`SADS: Unmapped SADS property '${respSadsPropKey}' in responsive rule for ${targetSelector}`);
        }
      });
    } catch (e) {
  ...
  private _generateBaseCss(attributes: SadsElementDataset, targetSelector: string): string {
    let baseCssText = "";
    for (const attrKey in attributes) {
      if ( Object.prototype.hasOwnProperty.call(attributes, attrKey) && attrKey.toLowerCase().startsWith("sads") &&
           attrKey.toLowerCase() !== "sadsresponsiverules" && attrKey.toLowerCase() !== "sadscomponent" && attrKey.toLowerCase() !== "sadselement") {
        ...
        if (cssProperty && actualValue !== null) baseCssText += `${cssProperty}: ${actualValue};\n`;
        else if (!cssProperty && sadsPropertyKey.toLowerCase() !== "layouttype") console.warn(`SADS: Unmapped SADS property '${sadsPropertyKey}' for ${targetSelector}`);
      }
    }
    return baseCssText;
  }
  ...
  private _addCssRule(selector: string, rules: string, mediaQuery: string | null = null): void {
    ...
    try {
      ...
      if (this.dynamicStyleSheet && this.dynamicStyleSheet.ownerNode) console.error(`SADS: Stylesheet ownerNode content at time of error:`, this.dynamicStyleSheet.ownerNode.textContent);
      else if (!this.dynamicStyleSheet) console.error(`SADS: Dynamic stylesheet was null at time of error.`);
    }
  }
  ...
  private _mapSadsPropertyToCss(sadsPropertyKey: string): string | null {
    ...
    const propertyMap: { [key: string]: string | null } = {
      "bg-color": "background-color", "text-color": "color", "font-size": "font-size", "font-weight": "font-weight", "font-style": "font-style",
      "text-align": "text-align", "text-decoration": "text-decoration", "border-radius": "border-radius", "border-width": "border-width",
      "border-style": "border-style", "border-color": "border-color", "max-width": "max-width", width: "width", height: "height",
      display: "display", "flex-direction": "flex-direction", "flex-wrap": "flex-wrap", "flex-justify": "justify-content",
      "justify-content": "justify-content", "flex-align-items": "align-items", "align-items": "align-items", "flex-basis": "flex-basis",
      gap: "gap", shadow: "box-shadow", "object-fit": "object-fit", padding: "padding", "padding-top": "padding-top",
      "padding-bottom": "padding-bottom", "padding-left": "padding-left", "padding-right": "padding-right", margin: "margin",
      "margin-top": "margin-top", "margin-bottom": "margin-bottom", "margin-left": "margin-left", "margin-right": "margin-right",
      resize: "resize", cursor: "cursor", transition: "transition", "box-sizing": "box-sizing", "layout-type": null,
    };
    if (propertyMap.hasOwnProperty(kebabKey)) return propertyMap[kebabKey];
    return kebabKey;
  }

  private _mapSemanticValueToActual(cssProperty: string | null, semanticValue: string | undefined | null): string | null {
    ...
    if (valueStr.startsWith("custom:")) return valueStr.substring("custom:".length);
    const isDarkMode = typeof document !== "undefined" && document.body.classList.contains("dark-mode");
    const themeCategoryMap: { [cssProp: string]: keyof SadsTheme | null } = {
      padding: "spacing", "padding-top": "spacing", "padding-bottom": "spacing", "padding-left": "spacing", "padding-right": "spacing",
      margin: "spacing", "margin-top": "spacing", "margin-bottom": "spacing", "margin-left": "spacing", "margin-right": "spacing",
