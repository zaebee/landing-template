...
class TestBuildScript(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.portfolio_items_data = [
            {"title_i18n_key": "portfolio_title_1", "desc_i18n_key": "portfolio_desc_1", "img_src": "img1.jpg", "img_alt": "Alt 1"},
            {"title_i18n_key": "portfolio_title_2", "desc_i18n_key": "portfolio_desc_2", "img_src": "img2.jpg", "img_alt": "Alt 2"},
        ]
        with open(os.path.join(self.test_data_dir, "portfolio.json"), "w", encoding="utf-8") as f:
            json.dump(self.portfolio_items_data, f)

        self.blog_posts_data = [
            {"title_i18n_key": "blog_title_1", "excerpt_i18n_key": "blog_excerpt_1", "link": "link1.html", "cta_i18n_key": "blog_cta_1"},
            {"title_i18n_key": "blog_title_2", "excerpt_i18n_key": "blog_excerpt_2", "link": "link2.html", "cta_i18n_key": "blog_cta_2"},
        ]
        with open(os.path.join(self.test_data_dir, "blog.json"), "w", encoding="utf-8") as f:
    ...
    def tearDown(self):
        ...
        if os.path.exists("index_es.html"): os.remove("index_es.html")
    ...
    def test_load_translations_existing(self):
        ...
        with mock.patch('build.open', mock.mock_open(read_data=json.dumps(self.en_translations)), create=True) as mocked_open:
            translations = load_translations(f"{self.test_locales_dir}/en")
            self.assertEqual(translations, self.en_translations)
            mocked_open.assert_called_once_with(f"locales/{self.test_locales_dir}/en.json", "r", encoding="utf-8")

        with mock.patch('build.open', mock.mock_open(read_data=json.dumps(self.es_translations)), create=True) as mocked_open:
            translations = load_translations(f"{self.test_locales_dir}/es")
            self.assertEqual(translations, self.es_translations)
            mocked_open.assert_called_once_with(f"locales/{self.test_locales_dir}/es.json", "r", encoding="utf-8")
    ...
    def test_load_translations_invalid_json(self):
        ...
        with open(os.path.join(self.test_locales_dir, "invalid.json"), "w", encoding="utf-8") as f:
            f.write("{'greeting': 'Hello', 'farewell': ") # Invalid JSON

        with mock.patch('build.open', mock.mock_open(read_data="{invalid_json_content"), create=True) as mocked_open:
            translations = load_translations(f"{self.test_locales_dir}/invalid")
            self.assertEqual(translations, {})
            mocked_open.assert_called_once_with(f"locales/{self.test_locales_dir}/invalid.json", "r", encoding="utf-8")
    ...
    def test_translate_html_content_missing_key(self):
        ...
        html_content = '<p data-i18n="greeting">Greeting</p><p data-i18n="missing_key">Missing</p>'
        translated_html = translate_html_content(html_content, self.en_translations)
        self.assertIn(self.en_translations["greeting"], translated_html)
        self.assertIn("Missing", translated_html) # Key "missing_key" not in en_translations

    def test_load_dynamic_data_portfolio(self):
        """Test loading dynamic portfolio data."""
        items = load_dynamic_data(os.path.join(self.test_data_dir, "portfolio.json"), PortfolioItem)
        self.assertEqual(len(items), len(self.portfolio_items_data))
        if items: # mypy check
            self.assertIsInstance(items[0], PortfolioItem)
            self.assertEqual(items[0].title_i18n_key, self.portfolio_items_data[0]["title_i18n_key"])

    def test_load_dynamic_data_blog(self):
        """Test loading dynamic blog data."""
        posts = load_dynamic_data(os.path.join(self.test_data_dir, "blog.json"), BlogPost)
        self.assertEqual(len(posts), len(self.blog_posts_data))
        if posts: # mypy check
            self.assertIsInstance(posts[0], BlogPost)
            self.assertEqual(posts[0].title_i18n_key, self.blog_posts_data[0]["title_i18n_key"])
    ...
    def test_generate_portfolio_html(self):
        ...
        items = [PortfolioItem(title_i18n_key="p_title", desc_i18n_key="p_desc", img_src="img.png", img_alt="alt")]
        translations = {"p_title": "Translated Title", "p_desc": "Translated Description"}
    ...
    def test_generate_blog_html(self):
        ...
        posts = [BlogPost(title_i18n_key="b_title", excerpt_i18n_key="b_excerpt", link="link.html", cta_i18n_key="b_cta")]
        translations = {"b_title": "Blog Title", "b_excerpt": "Blog Excerpt", "b_cta": "Read More"}
    ...
    def test_main_function_creates_files(
        ...
        mock_load_trans.side_effect = [
            {"header_text": "EN Header", "footer_text": "EN Footer", "hero_title": "EN Hero"}, # en translations
            {"header_text": "ES Header", "footer_text": "ES Footer", "hero_title": "ES Hero"}  # es translations
        ]
        # Mock load_dynamic_data to return specific types
        mock_portfolio_item = PortfolioItem(title_i18n_key="p1", desc_i18n_key="d1", img_src="portfolio.jpg", img_alt="alt")
        mock_blog_post = BlogPost(title_i18n_key="b1", excerpt_i18n_key="e1", link="blog.html", cta_i18n_key="c1")
        ...
        def mock_file_open_side_effect(filename, *args, **kwargs):
            ...
            if filename.startswith("blocks/") and args[0] == "r":
                if "hero" in filename:
                    return mock.mock_open(read_data="<div data-i18n='hero_title'>Hero</div>").return_value
                if "portfolio" in filename:
                     return mock.mock_open(read_data="<div>{{portfolio_items}}</div>").return_value
                if "blog" in filename:
                     return mock.mock_open(read_data="<div>{{blog_posts}}</div>").return_value
        ...
        write_calls = [call for call in mock_file_open.call_args_list if call[1].get('mode') == 'w' or (len(call[0]) > 1 and call[0][1] == 'w')]
        ...
        self.assertTrue(any(cargs[0] == "index.html" and cargs[1] == "w" for cargs, ckwargs in mock_file_open.call_args_list if cargs))
        # For index_es.html
        self.assertTrue(any(cargs[0] == "index_es.html" and cargs[1] == "w" for cargs, ckwargs in mock_file_open.call_args_list if cargs))
